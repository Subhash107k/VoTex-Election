...
app.post("/api/auth/verify-email-code", (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Email address successfully verified via simulated secure SMTP.",
    });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
app.post("/api/auth/register", (req, res) => {
  try {
    ...
    // Welcome email logs (Step 2 specs) — include direct login button and next steps
    const frontendUrl = (
...
app.post("/api/fingerprint/validate", authenticateToken, (req: any, res) => {
  try {
    ...
    const matches = profiles
      .filter(
        (profile) => profile.userId !== req.user.id && profile.fingerprintImage,
      )
      .map((profile) => {
        const storedHash =
...
app.post("/api/profile/complete", authenticateToken, (req: any, res) => {
  try {
    ...
    const isFaceDuplicate = users.some((u) => {
      ...
      if (!u.faceTemplate || !faceTemplateArray || u.faceTemplate.length === 0)
        return false;
      let sumSq = 0;
