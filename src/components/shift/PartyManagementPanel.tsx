...
const PRESET_LOGOS = [
  {
    name: "Sun Emblem",
    url: "https://images.unsplash.com/photo-1544383835-bda2bc66a55d?auto=format&fit=crop&q=80&w=150",
  },
  {
    name: "Sovereign Tree",
  ...
  {
    name: "Progress Bell",
    url: "https://images.unsplash.com/photo-1520690214124-2405c5217036?auto=format&fit=crop&q=80&w=150",
  },
  {
    name: "Solidarity Fist",
    url: "https://images.unsplash.com/photo-1595152772835-219674b2a8a6?auto=format&fit=crop&q=80&w=150",
  },
  {
    name: "Democratic Star",
    url: "https://images.unsplash.com/photo-1603504824368-2b821dfbb25e?auto=format&fit=crop&q=80&w=150",
  },
];
...
export const PartyManagementPanel: React.FC<PartyManagementPanelProps> = ({
  ...
  const isDescVal = partyForm.description
    ? partyForm.description.trim() !== ""
    : false;
  ...
  const errors = {
    name:
      touched.name && !isNameVal
        ? "Party classification name is mandatory."
        : "",
    code:
      touched.code && !isCodeVal ? "Party code/abbreviation is mandatory." : "",
    description:
      touched.description && !isDescVal
        ? "Manifesto description is mandatory."
        : "",
    logoUrl:
      touched.logoUrl && !isLogoVal
        ? "Please select a quick emblem or upload a party logo."
        : "",
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = partyForm.id
        ? `/api/parties/${partyForm.id}`
        : "/api/parties";
  ...
  const handleDeleteClick = async (id: string, name: string) => {
    ...
    if (activeCandidates.length > 0) {
      triggerToast(
        `Cannot delete party: "${name}" has ${activeCandidates.length} active registered candidate(s) linked.`,
        true,
      );
      return;
    }
    ...
    if (
      !window.confirm(
        `Are you sure you want to permanently delete the party "${name}"? This action cannot be undone.`,
      )
    ) {
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Configure democratic party credentials, secure physical symbol
            assets, and coordinate with federal candidates.
          </p>
        </div>
      ...
      >
        <AnimatePresence>
          {filteredParties.map((p) => {
            const affiliatedCands = candidates.filter(
              (c) => c.party === p.name || c.party === p.code,
            );
            return (
              <motion.div
                layout
              ...
              >
                ...
                <div className="space-y-3">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0">
                      <h4
                        className="font-extrabold text-slate-900 dark:text-white text-sm tracking-tight leading-snug line-clamp-2"
                        title={p.name}
                      >
                        {p.name}
                      </h4>
                ...
                <div className="text-[11px] space-y-2 border-t border-slate-100 dark:border-slate-800 pt-3 text-slate-600 dark:text-slate-450">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span
                      className="font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[170px]"
                      title={p.headquarters}
                    >
                      {p.headquarters || "N/A"}
                    </span>
                  </div>
        ...
        {filteredParties.length === 0 && (
          <div className="col-span-full py-16 text-center text-slate-400 bg-white dark:bg-slate-900 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
            ...
            <p className="font-bold text-sm text-slate-600 dark:text-slate-300">
              No parties match search queries
            </p>
            <p className="text-xs text-slate-400 mt-1">
              Initialize or formulate a political party directory entry above.
            </p>
          </div>
        )}
      </motion.div>
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3 mb-5">
                <h4 className="font-extrabold text-slate-900 dark:text-white text-base flex items-center gap-2">
                  ...
                  <span>
                    {partyForm.id
                      ? "Alter Political Party Credentials"
                      : "Enroll New Political Party"}
                  </span>
                </h4>
                <button
              ...
              <form
                ...
                className="flex flex-col gap-4 text-xs font-sans text-slate-700 dark:text-slate-300"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="sm:col-span-2">
                    <label className="flex items-center justify-between text-slate-500 dark:text-slate-400 font-bold mb-1.5 uppercase text-[10px] tracking-wide">
                      ...
                      {errors.name && (
                        <span className="text-[9px] text-red-500 font-medium lowercase font-sans">
                          {errors.name}
                        </span>
                      )}
                    </label>
                    <input
                      ...
                      className={`w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border rounded-xl text-slate-850 dark:text-slate-100 focus:ring-1 focus:ring-blue-500 focus:outline-none transition-colors ${
                        errors.name
                          ? "border-red-500 bg-red-50/5 dark:bg-red-950/5 focus:border-red-500 focus:ring-red-500"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border rounded-xl text-slate-850 dark:text-slate-100 focus:ring-1 focus:ring-blue-400 focus:outline-none transition-colors ${
                        errors.code
                          ? "border-red-500 bg-red-50/5 dark:bg-red-950/5 focus:border-red-500 focus:ring-red-550"
                          : "border-slate-200 dark:border-slate-700 focus:border-blue-500"
                      }`}
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPartyForm({ ...partyForm, leader: e.target.value })
                      }
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-slate-500 dark:text-slate-400 font-bold uppercase text-[10px] tracking-wide">
                    ...
                    {errors.logoUrl && (
                      <span className="text-[9px] text-red-500 font-semibold lowercase font-sans">
                        {errors.logoUrl}
                      </span>
                    )}
                  </div>
                      ...
                      >
                        ...
                        <div className="text-left space-y-1">
                          ...
                          <p
                            ...
                            title={partyForm.logoUrl}
                          >
                            Source:{" "}
                            {partyForm.logoUrl.startsWith("data:")
                              ? "Local File Data (Base64)"
                              : "Network Asset Link"}
                          </p>
                          <button
                            type="button"
                      ...
                      <>
                        ...
                        <div>
                          <p className="font-bold text-slate-850 dark:text-slate-200">
                            Click or Drag & Drop to Upload Emblem File
                          </p>
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            Supports PNG, JPG, JPEG standard graphic definitions
                          </p>
                        </div>
                      </>
                    )}
                  ...
                  <div className="mt-1">
                    <span className="text-[9px] text-slate-400 block mb-1">
                      Or input custom image web address link manually:
                    </span>
                    <input
                      type="text"
                ...
                <div className="bg-slate-50 dark:bg-slate-800/65 border border-slate-200 dark:border-slate-800 p-3 rounded-xl flex gap-2.5 text-[11px] text-slate-500 dark:text-slate-400 leading-normal">
                  ...
                  <p>
                    All configurations registered here undergo standard
                    democratic oversight. If modified, affiliated candidates on
                    active ballots automatically inherit updated logo and
                    abbreviation codes without manual conflict resolution.
                  </p>
                </div>
                ...
                {!isFormValid && (
                  <div className="text-center">
                    <span className="text-[10px] text-amber-500 dark:text-amber-400 font-semibold flex items-center justify-center gap-1">
                      ...
                      <span>
                        Confirm button is locked until all required fields (*)
                        are completed.
                      </span>
                    </span>
                  </div>
