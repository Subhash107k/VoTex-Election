...
--- | --- | --- | --- | ---
GET | `/api/system/dispatches` | Admin roles | View email/SMS dispatch log | Now protected; still mask OTP content before broader production use.
POST | `/api/system/dispatches/clear` | Admin roles | Clear dispatch log | Now protected; should also be audited.
...
GET | `/api/dashboard/stats` | Admin/officer roles | Dashboard stats | Now protected; add pagination/aggregation optimizations.
GET | `/api/notifications` | JWT | Notifications | Now filtered by target user/role where fields are present.
...
GET | `/api/secops/db-status` | Admin/officer roles | DB status | Now role-protected.
